...
def _unit_ball_volume(dim):
    # c_d = volume of the unit Euclidean d-ball
    return math.pi**(dim/2) / spl.gamma(dim/2 + 1)

def kl(y, n=None, k=1, shuffle=True, standardize=False, rng=np.random):
    """
    Kozachenko–Leonenko entropy estimator (Euclidean / L2).
    Matches the canonical form used by FNN:
      H ≈ ψ(N) - ψ(k) + log c_d + d * mean(log r_k)
    where r_k is the Euclidean distance to the k-th NN and c_d is the unit d-ball volume.
    """
    ...
    nbrs = NearestNeighbors(n_neighbors=k+1, algorithm='auto',
                            metric='minkowski', p=2).fit(y)
    dist, _ = nbrs.kneighbors(y)
    r = np.maximum(dist[:n, k], 1e-12)  # guard ties/duplicates

    log_cd = math.log(_unit_ball_volume(dim))
    hh = dim * np.log(r) + log_cd
    if standardize:
